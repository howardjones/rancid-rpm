...
Version:        3.8
Release:        1%{?dist}
Summary:        Really Awesome New Cisco confIg Differ with Subversion and extra devices

Prefix: /opt/rancid
%define _prefix /opt/rancid
...
%prep
%setup -q -n rancid-%{version}

%build
%configure --prefix=%{_prefix} --with-svn
...
mkdir $RPM_BUILD_ROOT/%{_prefix}/var
...
cp cloginrc.sample $RPM_BUILD_ROOT/%{_prefix}/.cloginrc

%pre
if [ $1 -eq 1 ]; then
   /usr/bin/getent passwd rancid || useradd -M -r -d %{_prefix} -c "RANCID User" rancid
fi
...
%dir %attr(750,rancid,rancid) %{_prefix}
# %dir %attr(750,rancid,rancid) %{_prefix}/var
%{_prefix}/*
%config(noreplace) %attr(640,rancid,rancid) %{_prefix}/.cloginrc
%config(noreplace) %attr(640,rancid,rancid) %{_prefix}/etc/rancid.conf
%config(noreplace) %attr(640,rancid,rancid) %{_prefix}/etc/rancid.types.conf
%config %attr(640,rancid,rancid) %{_prefix}/etc/rancid.types.base
